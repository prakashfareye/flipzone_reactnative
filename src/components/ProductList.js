...
const ProductList = ({navigation, route}) => {
    ...
    AsyncStorage.getItem('ProductsByCategory')
      .then(result => {
        if (result != undefined) {
          ...
          AsyncStorage.getItem('similarProducts').then(result => {
            if (result != undefined) {
              console.log(result);
              setTitle(result);
              fetch('http://10.0.2.2:8085/product/search/' + result, {
                ...
                .catch(error =>
                  console.log('get similarProducts api fail ', error),
                );
            }
          });
        }
      ...
      {!isSort && (
        <View style={{justifyContent: 'space-evenly', flexDirection: 'column'}}>
          <TouchableOpacity
            onPress={() => typePress(1)}
            style={{
              backgroundColor: filterBy == 1 ? ProjectColors.navy : 'white',
              height: 50,
              width: 200,
          ...
          <TouchableOpacity
            onPress={() => typePress(2)}
            style={{
              backgroundColor: filterBy == 2 ? ProjectColors.navy : 'white',
              height: 50,
              width: 200,
              justifyContent: 'center',
              alignSelf: 'center',
      ...
      {isSort && (
        <View style={{justifyContent: 'space-evenly', flexDirection: 'column'}}>
          <TouchableOpacity
            onPress={() => typePress(1)}
            style={{
              backgroundColor: sortBy == 1 ? ProjectColors.navy : 'white',
              height: 50,
              width: 200,
              justifyContent: 'center',
              alignSelf: 'center',
          ...
          <TouchableOpacity
            onPress={() => typePress(2)}
            style={{
              backgroundColor: sortBy == 2 ? ProjectColors.navy : 'white',
              height: 50,
              width: 200,
              justifyContent: 'center',
              alignSelf: 'center',
  ...
  const typePress = type => {
    if (isSort) {
      ...
      if (filterBy == type) {
        setFilterBy(0);
        let sortedProducts = [...fullProducts];
        if (sortBy == 1) {
          sortedProducts.sort((a, b) => a.productPrice - b.productPrice);
        } else if (sortBy == 2) {
          sortedProducts.sort((a, b) => b.productPrice - a.productPrice);
  ...
  const productCardPress = index => {
    ...
    AsyncStorage.setItem('ProductFeatures', JSON.stringify(products[index]))
      ...
      .catch(error =>
        console.log('productCardPress AsyncStorage error', error),
      );
  };
