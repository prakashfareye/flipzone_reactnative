...
const Transaction = ({route, navigation}) => {
    ...
    const handleSubmit = () => {
        ...
        .then((userResult) => {
            userResult = JSON.parse(userResult)
            if(userResult != undefined) {
                AsyncStorage.getItem('productTransaction')
                .then((productTransactionResult) => {
                    if(productTransactionResult != undefined) {
                            ...
                            "orderItems": [
                                {
                                    ...
                                    "total": productTransactionResult.productItemCount*productTransactionResult.productPrice
                                }
                            ],
                            "user": {
                                "userId": userResult.userId
                        ...
                        fetch('http://10.0.2.2:8085/order', {
                            ...
                            .catch(error => console.log('summary transaction api fail', error));
                        ...
                        .then((cartTransactionResult) => {
                            if(cartTransactionResult != undefined) {
                                ...
                                cartTransactionResult.forEach(element => {
                                    total += element.cartItemQuantity*element.cartItemPrice
                                    orderItems.push({
                                ...
                                fetch('http://10.0.2.2:8085/order', {
                                    ...
                                    .catch(error => console.log('cart transaction api fail', error));

                                }
                            })
                                
                            .catch(error => console.log("cart AsyncStorage  error", error));
            }
